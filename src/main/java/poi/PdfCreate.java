...
public class PdfCreate {
    ...
    static {
        ...
        try {
            STSONGFont = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED);
        } catch (Exception e) {
    ...
    public void creatPdf() throws Exception {
        FileOutputStream outputStream = new FileOutputStream("D:\\Test\\pdf_02.pdf");
        Document document = new Document(PageSize.A4);
        PdfWriter writer = null;
        writer = PdfWriter.getInstance(document, outputStream);
        document.open();
        document.add(this.getTitleTable());
        createHtml(outputStream);


    }
    private Table getTitleTable() throws Exception {
        Table title = new Table(4, 1);
        title.setSpacing(10);
        title.setBorder(Table.NO_BORDER);
        title.getDefaultCell().setBorder(Cell.NO_BORDER);
        Cell titleCell = new Cell();
        titleCell.setColspan(4);
        titleCell.add(new Chunk("京东海关拍卖平台拍卖专场信息表", new Font(STSONGFont, TITLE_FONT_SIZE,                Font.BOLD)));
        titleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        title.addCell(titleCell);
        return title;
    }
    public  void createHtml(FileOutputStream outputStream) throws IOException {
